...
| `cache.singleflight` | `false` | Coalesce concurrent identical cacheable misses onto ONE upstream call (#57, ADR-0008): the first is the leader, later identical arrivals are followers marked `X-Cradle-Flight: follower`. **Default OFF — DO NOT ENABLE YET.** The pre-merge review of #58 found three CRITICAL concurrency defects that shipped unfixed: #63 (stream open-failure poisons the key ~120s), #64 (registry pop clobbers a replacement flight), #65 (healthy long/slow streams treated as stale — makes #64 reachable on the happy path). These are latent only because the flag is off. Enabling before #63/#64/#65 are fixed will cause hung requests and duplicate upstream calls, and a soak run with the flag on will hit the bugs and misreport. After the fixes: opt in only when a soak shows `cradle_flight_aborts_total{reason="timeout"\|"stale"}` flat and `cradle_flight_followers_total > 0` under a concurrent-identical burst. In intercept mode a burst only coalesces if the clients share an API key (the principal is derived from `Authorization`). |
