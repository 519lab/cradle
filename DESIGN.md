...
| 22 | Rerank latency budget | **On CPU the reranker breaks the G3 25 ms L2 p99 budget** (~15–40 ms/hit for bge-reranker-base). `l2.rerank_device: cuda` (extra `cradle[rerank-gpu]`) restores it (~2–5 ms). Accepted: correctness over the CPU latency target; the budget in **G3 applies to the cosine+embed path**, and is met with rerank on **only on a GPU host**. | The 25 ms figure predates the FP fix. Serving a wrong answer fast is worse than a correct answer in 40 ms — still ~200× under a 5–10 s upstream miss. CPU default keeps the thin deploy; GPU is the opt-in path back under budget. |
