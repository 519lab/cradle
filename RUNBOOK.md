...
| `cradle_flight_aborts_total{reason}` | Single-flight leaders that failed their followers, by cause: `upstream_error` (backend errored mid-stream **or at stream open**, #63), `truncated_stream`, `unexpected_tool_call`, `leader_disconnect` (leader's client vanished), `timeout` (follower saw no progress from its leader for `upstream.timeout_s`), `stale` (a leader made no progress for `upstream.timeout_s` and was either replaced by the next identical arrival or swept by the background **flight reaper** — #67 evicts a leak even when no arrival replaces it, e.g. an unstarted stream generator whose `finally` never ran; #65 measures staleness from last publish, not creation, so a healthy long/slow stream no longer counts). `upstream_error`/`truncated_stream`/`leader_disconnect` are expected at low rates; climbing `timeout` or `stale` means leaders are genuinely wedging — investigate before flipping `singleflight` to default-on. The reaper runs only when `cache.singleflight` is on and sweeps every `upstream.timeout_s`. |
