...
| `cradle_cache_write_errors_total` | A cacheable response whose **writeback raised** — an L1 (diskcache) or L2 (Qdrant) backend error (#70). Distinct from a write *skip* (a policy decision): the answer was already served to the client (and to single-flight followers) intact; only the cache write failed, so it is counted + logged, not raised. **Should stay ~flat.** A climbing rate means the cache is silently not filling (hit rate will sag while answers keep flowing) → check L1/L2 backend health (disk full/permissions for diskcache; Qdrant reachable/healthy). Correlate with the exception traceback on the `cradle.writeback` logger. |
...
| `cradle_flight_followers_total` | If `cache.singleflight` on: requests coalesced onto a leader's upstream call — each is an upstream call (and writeback) **saved**. Zero under load means bursts are not identical/homogeneous, or the flag is off. **Known limitation (#71):** in a simultaneous first-tick burst the arrivals still coalesce to one upstream call, but each runs its own embed + L2 query before the leader registers (only arrivals *after* the leader registers skip that work), so N embeds for one upstream call in a burst is expected, not a bug. Watch the embed-stage latency (`cradle_latency_seconds{stage="embed"}`) under real bursts during the soak; embed-pool saturation there is what would justify closing #71. |
| `cradle_flight_aborts_total{reason}` | Single-flight leaders that failed their followers, by cause: `upstream_error` (backend errored mid-stream **or at stream open**, #63), `truncated_stream`, `unexpected_tool_call`, `leader_disconnect` (leader's client vanished), `timeout` (follower saw no progress from its leader for `upstream.timeout_s`), `stale` (either the register seam replaced a stale slot on the next identical arrival, or the background **flight reaper** swept a leaked *unstarted stream generator* — #67/#76). The reaper is deliberately narrow: it only evicts a **stream** flight with **zero published frames** (the sole leak shape — a generator Starlette cancelled before iterating, so its `finally` never ran); a started stream leader has published frames, and a JSON leader always resolves in its own `finally`, so neither is ever reaped even when quiet (#76 — the reaper must not fail a live leader out from under its followers). It also closes that leak's orphaned upstream response (#77). `upstream_error`/`truncated_stream`/`leader_disconnect` are expected at low rates; climbing `timeout` or `stale` means leaders are genuinely wedging — investigate before flipping `singleflight` to default-on. The reaper runs only when `cache.singleflight` is on and sweeps every `upstream.timeout_s`. |
...
| All requests are `MISS`/`BYPASS`, cache never fills | `X-Cradle-Cache`; `cradle_cache_write_skips_total` | Requests uncacheable: `temperature > cache.max_temperature`, `n!=1` / `stream+logprobs` (bypass), `cache_tool_streams:false` (tool streams bypass), or write-skips. Note tool streams that emit a tool call are MISS-not-cached by design (#43). | Confirm client params; bypass is by design for those shapes. Ensure `cache.cache_tool_streams` is on if the chat path uses tools. |
...
| `cache.singleflight` | `false` | Coalesce concurrent identical cacheable misses onto ONE upstream call (#57, ADR-0008): the first is the leader, later identical arrivals are followers marked `X-Cradle-Flight: follower`. **Default OFF — soak required before enabling.** The three CRITICAL concurrency defects the #58 review found are now **fixed** (#63 stream open-failure resolves the flight; #64 identity-safe registry release; #65 progress-based staleness), but the flag stays default-off until a LAN soak against the test container shows `cradle_flight_aborts_total{reason="timeout"\|"stale"}` flat and `cradle_flight_followers_total > 0` under a concurrent-identical burst. In intercept mode a burst only coalesces if the clients share an API key (the principal is derived from `Authorization`). |
